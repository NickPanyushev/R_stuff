...
Пересечем бины с помощью foverlaps
```{r}
setkeyv(all_bins, c("Chromosome", "Peak_Bins"))
foverlaps(all_bins[Cell_line == "A549"],
          all_bins[Cell_line == "HUVEC"])

#Reduce(foverlaps, all_bins )
...
length(unique(common_peaks$Peak_name))

common_peaks[, .N, by = .(Chromosome, START)]

common_peaks[, length(STOP - START), by = Cell_line]
common_peaks[, max(STOP - START), by = Cell_line]
