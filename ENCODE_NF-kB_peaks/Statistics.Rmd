...
Теперь сделаем датафрейм такой же, как мы делали для пересечений линий, и но количество пересечений будет отличаться от реальных данных - их будет везде по 500
```{r}
...
ggplot(all_df[Line_number > 1], aes(x = Line_number, y = Bin_numbers))+
  geom_jitter(aes(color = type), position = position_jitterdodge(jitter.width = 0.1), size = 2, alpha = 0.5, na.rm = T) + 
  labs(title = "Common bin number",
       x = "Number of lines",
       y = "Common peak number")+
   ylim(0, 1e+05)+
