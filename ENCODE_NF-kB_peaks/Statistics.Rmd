...
generate_vector <- function(df, piece_size, bin_size = 50){
  ...
  return(sort(out_vector))
}

class(generate_vector(to_generate_dt, piece_size))
...
intersect_matrix <- apply(intersect_matrix, 2, function(x) generate_vector(to_generate_dt, piece_size)) #Исправить так, чтобы матрица была, а не лист
```
