...
Теперь сделаем датафрейм такой же, как мы делали для пересечений линий, с тем лишь отличием, что для каждого пересечения у нас будет 100 точек
```{r}
...
ggplot(all_df[Line_number>=2, ], aes(x = Line_number, y = Bin_numbers, group = type))+
  geom_col(position = "dodge", alpha = 0.5, size = 1.5, na.rm = T)+
  #position_dodge()+
  labs(title = "Common bin number",
       x = "Number of lines",
       y = "Common peak number")+
...
```{r}
t.test(comb_df[comb_df$Line_number==6, Bin_numbers], generated_df[generated_df$Line_number==6, Bin_numbers], alternative = "greater")
```
...
Как говорит wolfram alpha, у нас тут наблюдается логарифмическая зависимость
Ну убедимся в этом!
...
fit_natural <- loglm( ,comb_df[, round(mean(Bin_numbers), digits = 0), by = Line_number])
...
x <- seq(from=1,to=20)
y <- predict(fit_natural, interval="confidence")
