...
Надо эту функцию отладить, она не работает
...
bin.peaks <- function(dataframe, window_size = 50, append = TRUE){
  fun <- function(Start, Stop){
    ...
    for (i in range){
      append(position, position[tail(position, 1)]+window_size)
    }
  return(position)}
  ...
  if (append == TRUE){
    ...
    bin_vector <- vector(dataframe, fun)
    return(bin_vector)
  }
}
bin.peaks(HeLa_peaks, append = FALSE)
```
