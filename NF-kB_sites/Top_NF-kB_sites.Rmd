...
Бинаризуем пики окошком в 50 п.о.
...
bin.peaks <- function(dataframe, window_size = 50, append = TRUE){
  ...
  fun <- function(Start, Stop){ 
    position <- Start%/%window_size*window_size
    range <- round(abs(Stop-Start)/window_size, digits = 0) 
    for (i in seq(position, range, by=window_size))
      {
  ...
  if (append == TRUE){
    dataframe$peak_bins <- sapply (dataframe,
                                  fun(Start = dataframe$Start, Stop = dataframe$Stop)) 
    return(dataframe)
  }else{
    bin_vector <-  vector(mode="numeric", length=0)
    bin_vector <- append(bin_vector, )
    bin_vector <- sort(bin_vector)
    return(bin_vector)
  }
}
bin.peaks(HeLa_peaks, append = TRUE)
```
...
apply(HeLa_peaks[1,2:3 ], 1, function(x) fun(Start = x[1], Stop = x[2]))
class(HeLa_peaks$Start)
