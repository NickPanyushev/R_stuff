...
if (!file.exists("RM_rawout_RepMod.zip")){
  unzip("f.hep_RM_RepMod.out.zip")
  RM_rawout <- fread("fasciola_hepatica_RM_RepMod.fa.out" , sep = " ", blank.lines.skip = T, fill = T)
  ...
  zip("RM_rawout_RepMod.zip", "RM_rawout.tsv")
  file.remove("RM_rawout.tsv")
  file.remove("fasciola_hepatica_RM_RepMod.fa.out")
}else{
  unzip("RM_rawout_RepMod.zip")
