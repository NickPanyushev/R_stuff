...
table1 <- read.csv('/home/nickolay/ACTN4_projects/R_stuff/bradford_060317.17.txt', header = F) # считали из файла
...
samples <- c(1.233, 1.063, 1.193, 1.216, 1.112)

concentrations_EGG <- coeffs_EGG[1] + coeffs_EGG[2]*samples
concentrations_BSA <- coeffs_BSA[1] + coeffs_BSA[2]*samples

for_WB <- data.frame(Names = c("H1299", "sh1", "sh3", "sh3.2", "sh4.2"),
                     concentrations_BSA)


for_WB$Concentrations <- (as.numeric(for_WB$concentrations_BSA)/1000)
total_volume <- 50 #объем пробы полный
